...
//            self?.userLocationforPost = self?.mapView.userLocation.coordinate
        ...
        button_myLocation = make_floating_button(Constants.Assetname.Images.Symbol.GPS,
            ...
            [weak self] in
            if let userLocation = self?.mapView.mapView.locationOverlay.location{
...
//네이버 맵 마커 커스텀 클래스 가능 테스트 코드
extension VC_main{
    
    func showNMapUserLocation(){
        
//        let myLocationImage = UIImage(named: "symbols_mylocation")!
        mapView.mapView.positionMode = .normal
        let locationOverlay = mapView.mapView.locationOverlay
        locationOverlay.hidden = false
        locationOverlay.icon = NMFOverlayImage(name: "symbols_mylocation")
    }
    
    func createCustomNMapMarker(){
